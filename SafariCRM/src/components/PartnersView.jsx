...
export default function PartnersView({ partners, setPartners, bookings, packages = [] }) {
  ...
  const [prevPendingBalance, setPrevPendingBalance] = useState(0);
  ...
  useEffect(() => {
    ...
    // Sum B2B Cost item-by-item based on B2B rates
    const totalGross = filteredBookings.reduce((sum, b) => {
      const pkgObj = activePackages.find(p => p.name === b.packageName);
      if (!pkgObj) return sum + (parseFloat(b.price) || 0);
      
      let b2bRate = pkgObj.rate;
      if (partner.packages && partner.packages[pkgObj.id] !== undefined) {
        b2bRate = parseFloat(partner.packages[pkgObj.id]);
      } else {
        b2bRate = b.pricingType === 'offpeak' ? (pkgObj.offpeakRate || pkgObj.rate) : (pkgObj.peakRate || pkgObj.rate);
      }
      
      let cost = 0;
      if (pkgObj.type === 'per_person') {
        cost = b2bRate * (parseInt(b.pax) || 0);
      } else {
        const cars = Math.ceil((parseInt(b.pax) || 0) / 6) || 1;
        cost = b2bRate * cars;
      }
      cost += parseFloat(b.addonPrice) || 0;
      return sum + cost;
    }, 0);
    ...
    const currentMonthTotal = includeVat ? grossWithVat : totalGross;
    const totalPending = parseFloat(prevPendingBalance) + currentMonthTotal - parseFloat(amountReceived);
    ...
    setInvoiceData({
      ...
      grossWithVat,
      currentMonthTotal,
      prevPendingBalance: parseFloat(prevPendingBalance) || 0,
      amountReceived: parseFloat(amountReceived) || 0,
      totalPending,
      invoiceNumber: `INV-${partner.id.toUpperCase().slice(0, 3)}-8034`,
      issueDate: new Date().toISOString().split('T')[0]
    });
  }, [invoicePartnerId, invoiceStartDate, invoiceEndDate, bookings, partners, prevPendingBalance, amountReceived, includeVat]);
              ...
              {partners.map(p => {
                ...
                return (
                  <div key={p.id} style={{ 
                    ...
                    <div style={{ borderTop: '1px solid rgba(0,0,0,0.04)', marginTop: '14px', paddingTop: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
          ...
          {selectedPartnerForOverrides && (
            <div className="panel-card" style={{ alignSelf: 'start' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
                <div className="panel-title" style={{ margin: 0 }}>
                  <span style={{ fontSize: '15px' }}>Custom Package B2B Rates: {selectedPartnerForOverrides.name}</span>
                </div>
              ...
              <p style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '16px', lineHeight: '1.4' }}>
                Specify partner-specific B2B rates (in AED) for individual packages here. If left blank, B2B calculations will fallback to the package's default retail rate.
              </p>
              ...
              <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
                ...
                >
                  <option value="">-- Add Package to Partner --</option>
                  {activePackages
                    .filter(pkg => selectedPartnerForOverrides.packages?.[pkg.id] === undefined)
                    .map(pkg => (
                ...
                <button 
                  ...
                  onClick={() => {
                    const selectEl = document.getElementById('add-package-select');
                    const pkgId = selectEl.value;
                    if (!pkgId) return;
                    
                    const targetPkg = activePackages.find(p => p.id === pkgId);
                    if (!targetPkg) return;
                    
                    handleUpdateOverride(pkgId, targetPkg.rate); // Add with default rate
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '500px', overflowY: 'auto', paddingRight: '4px' }}>
                {activePackages
                  .filter(pkg => selectedPartnerForOverrides.packages?.[pkg.id] !== undefined)
                  .map(pkg => {
                    const overrideVal = selectedPartnerForOverrides.packages[pkg.id];
                      ...
                      <div key={pkg.id} style={{ 
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                            <input 
                              type="number"
                              placeholder={pkg.rate}
                              className="form-control"
                              style={{ width: '95px', padding: '6px 36px 6px 8px', fontSize: '12.5px', textAlign: 'right' }}
                              value={overrideVal}
                              onChange={(e) => handleUpdateOverride(pkg.id, e.target.value)}
                            />
                            <span style={{ position: 'absolute', right: '8px', fontSize: '11px', color: 'var(--text-muted)', pointerEvents: 'none' }}>AED</span>
                          </div>

                          <button 
                            type="button"
                            className="btn btn-secondary"
                            style={{ padding: '6px 8px', color: '#ef4444', borderColor: 'rgba(239, 68, 68, 0.2)', background: 'rgba(239, 68, 68, 0.05)', height: '34px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                            onClick={() => {
                              if (window.confirm(`Are you sure you want to remove "${pkg.name}" from this partner?`)) {
                                handleUpdateOverride(pkg.id, '');
                              }
                            }}
                            title="Delete package from partner"
                          >
                            <Trash2 size={13} />
                          </button>
                        </div>
                ...
                {(!selectedPartnerForOverrides.packages || Object.keys(selectedPartnerForOverrides.packages).length === 0) && (
                  <div style={{ textAlign: 'center', padding: '24px', color: 'var(--text-muted)', fontSize: '13px', background: '#faf6f0', borderRadius: '8px', border: '1px dashed #efe9df' }}>
                    No custom packages assigned to this partner. Use the selector above to assign packages.
                  </div>
                )}
              </div>
            </div>
              ...
              {invoiceData ? (
                <div id="printable-area-wrapper">
                  ...
                  <div className="premium-invoice-container" id="printable-area">
                    <div className="premium-invoice-header">
                      ...
                      <div className="premium-invoice-title-block">
                        ...
                        <div className="premium-balance-due-card">
                          ...
                          <span className="value">AED {invoiceData.totalPending.toLocaleString('en-AE', { minimumFractionDigits: 2 })}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <table className="premium-invoice-table">
                      <thead>
                        <tr>
                          ...
                          <th style={{ width: '130px', textAlign: 'right' }}>Amount (B2B)</th>
                        </tr>
                      </thead>
                      <tbody>
                        {invoiceData.bookings.map((b) => {
                          const pkgObj = activePackages.find(p => p.name === b.packageName);
                          let b2bRate = pkgObj ? pkgObj.rate : 0;
                          if (invoiceData.partner.packages && pkgObj && invoiceData.partner.packages[pkgObj.id] !== undefined) {
                            b2bRate = parseFloat(invoiceData.partner.packages[pkgObj.id]);
                          } else if (pkgObj) {
                            b2bRate = b.pricingType === 'offpeak' ? (pkgObj.offpeakRate || pkgObj.rate) : (pkgObj.peakRate || pkgObj.rate);
                          }
                          ...
                          if (pkgObj) {
                            if (pkgObj.type === 'per_person') {
                              b2bCost = b2bRate * (parseInt(b.pax) || 0);
                            } else {
                              const cars = Math.ceil((parseInt(b.pax) || 0) / 6) || 1;
                              b2bCost = b2bRate * cars;
                            }
                          } else {
                            b2bCost = parseFloat(b.price) || 0;
                          }
                          b2bCost += parseFloat(b.addonPrice) || 0;
                          ...
                          const descLine = `${b.customerName} (${formattedRowDate}) - ${b.packageName} - ${b.pickupLocation}${roomInfo}${b.addonName ? ` (Addon: ${b.addonName})` : ''}`;

                          return (
                            <tr key={b.id}>
                              <td className="desc-cell">{descLine}</td>
                              <td style={{ textAlign: 'center', fontWeight: 'bold' }}>{paxVal}</td>
                              <td style={{ textAlign: 'right', fontWeight: 'bold' }}>{b2bCost.toLocaleString()} AED</td>
                            </tr>
                          );
                        })}
                    ...
                    <div className="premium-summary-card" style={{ width: '100%', maxWidth: '400px', marginLeft: 'auto', display: 'flex', flexDirection: 'column', gap: '6px', padding: '12px', background: '#faf6f0', border: '1px solid #efe9df', borderRadius: '10px', marginTop: '16px' }}>
                      <div className="premium-summary-row" style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12.5px' }}>
                        <span>Previous Month Pending Balance:</span>
                        <strong>{invoiceData.prevPendingBalance.toLocaleString('en-AE', { minimumFractionDigits: 2 })} AED</strong>
                      </div>
                      <div className="premium-summary-row" style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12.5px', paddingTop: '4px', borderTop: '1px dashed #efe9df' }}>
                        <span>Current Month Services (Net B2B):</span>
                      ...
                      <div className="premium-summary-row" style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12.5px', fontWeight: 'bold' }}>
                        <span>Current Month Total Due:</span>
                        <strong>{invoiceData.currentMonthTotal.toLocaleString('en-AE', { minimumFractionDigits: 2 })} AED</strong>
                      </div>
                      <div className="premium-summary-row" style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12.5px', color: '#b91c1c', paddingTop: '4px', borderTop: '1px dashed #efe9df' }}>
                        <span>Amount Received (This Month):</span>
                        <strong>- {invoiceData.amountReceived.toLocaleString('en-AE', { minimumFractionDigits: 2 })} AED</strong>
                      </div>
                      <div className="premium-summary-row total" style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14.5px', fontWeight: '800', borderTop: '1.5px solid #8c5b30', paddingTop: '6px', marginTop: '4px', color: '#8c5b30' }}>
                        <span>TOTAL PENDING BALANCE:</span>
                        <strong>AED {invoiceData.totalPending.toLocaleString('en-AE', { minimumFractionDigits: 2 })}</strong>
                      </div>
                    </div>
