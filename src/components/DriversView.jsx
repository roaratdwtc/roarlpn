...
import { getWhatsAppConfirmationLink, getConfirmationText, getWhatsAppDriverLink, getBookingCampUse, getBookingQuadbike } from './BookingsView';
...
<body>
  ...
  <div class="driver-info">
    ...
    <div class="info-row"><span class="info-label">Daily Salary</span><span class="info-value">AED ${driver.defaultSalary || 100}</span></div>
  </div>
  ...
  <div class="summary-grid">
    ...
    <div class="summary-card">
      <span class="s-label">Petrol / Fuel</span>
      <span class="s-value">AED ${(stats.petrolSum || 0).toFixed(0)}</span>
    </div>
    <div class="summary-card">
      <span class="s-label">Salary Paid</span>
      <span class="s-value">AED ${(stats.salarySum || 0).toFixed(0)}</span>
    </div>
    <div class="summary-card highlight">
      <span class="s-label">Total Outstanding</span>
      <span class="s-value">AED ${((stats.totalCollected || 0) - (stats.salarySum || 0) - (stats.petrolSum || 0) - (stats.campUseSum || 0) - (stats.quadbikeSum || 0) - (stats.miscSum || 0)).toFixed(0)}</span>
    </div>
  </div>
  ...
  <table>
    ...
    <tbody>
      ...
      `).join('')}
      ${ledgerData.length === 0 ? '<tr><td colspan="5" style="text-align:center;color:#9ca3af;padding:16px">No performance records in this period.</td></tr>' : ''}
    </tbody>
  </table>
...
export default function DriversView({ drivers, setDrivers, bookings, expenses, packages = [], setActiveTab }) {
  ...
  const [ledgerStartDate, setLedgerStartDate] = useState('2026-06-01');
  const [ledgerEndDate, setLedgerEndDate] = useState('2026-07-31');
  ...
  const getDriverStats = (driverId) => {
    ...
    allDates.forEach(date => {
      ...
      if (dayExpenses.length > 0) {
        ...
        // No logged expense — shift-based defaults:
        ...
        // Salary and fuel per shift
        if (hasMorningShift) { salarySum += defaultSalary; petrolSum += 150; }
        if (hasEveningShift) { salarySum += defaultSalary; petrolSum += 150; }
      ...
      // Add quadbike expense always if configured (per bike/guest)
      dayBookings.forEach(b => {
  ...
  return (
    <div>
      ...
      {viewingDriver && (() => {
        ...
        const ledgerData = (() => {
          const driverBookings = bookings.filter(b => b.driverId === viewingDriver.id);
            ...
            .map(d => {
              ...
              if (dayExpenses.length > 0) {
                ...
                if (dayBookings.length > 0) {
                  daySalary = parseFloat(viewingDriver.defaultSalary) || 100;
                  const hasMorning = dayBookings.some(b => {
                    const nameLower = (b.packageName || '').toLowerCase();
                    return nameLower.includes('morning') || nameLower.includes('tour') || nameLower.includes('hatta') || nameLower.includes('city');
                  });
                  dayPetrol = hasMorning ? 150 : 50;

                  // Dynamic camp use from packages
                  dayCampUse = dayBookings.reduce((sum, b) => sum + getBookingCampUse(b, packages), 0);

                  // Dynamic quadbike expense from packages
                  const dayQuadbike = dayBookings.reduce((sum, b) => sum + getBookingQuadbike(b, packages), 0);
                  dayCampUse += dayQuadbike;
                }
              }
              ...
              <div className="modal-details-grid">
                ...
                <div className="modal-profile-card">
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '13px' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ color: 'var(--text-muted)', fontWeight: '600', fontSize: '11px' }}>DAILY SALARY</span>
                      <span style={{ fontWeight: '700' }}>AED {viewingDriver.defaultSalary || 100}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ color: 'var(--text-muted)', fontWeight: '600', fontSize: '11px' }}>DAILY FUEL COST</span>
                      <span style={{ fontWeight: '700' }}>AED {viewingDriver.defaultFuel || 150}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ color: 'var(--text-muted)', fontWeight: '600', fontSize: '11px' }}>PORTAL EMAIL</span>
                      <span style={{ fontWeight: '600', color: 'var(--primary)' }}>{viewingDriver.name.toLowerCase().replace(/\s+/g, '')}@roar.local</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="modal-profile-card">
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', fontSize: '13px', marginBottom: '12px' }}>
                    <div>
                      <span style={{ color: 'var(--text-muted)', fontSize: '11px', display: 'block' }}>PETROL FUEL</span>
                      <strong>AED {stats.petrolSum.toFixed(0)}</strong>
                    </div>
                    <div>
                      <span style={{ color: 'var(--text-muted)', fontSize: '11px', display: 'block' }}>SALARY PAID</span>
                    ...
                    <table className="modal-table">
                      <thead>
                        <tr>
                          ...
                          <th style={{ textAlign: 'right' }}>CASH COLLECTION</th>
                      ...
                      <tbody>
                        ...
                        {ledgerData.length === 0 && (
                          <tr>
                            <td colSpan="5" style={{ textAlign: 'center', padding: '16px', color: 'var(--text-muted)' }}>No performance history logged.</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
      ...
      {viewingLedgerDate && (() => {
        const dayBookings = bookings.filter(b => b.driverId === viewingDriver.id && b.date === viewingLedgerDate);
        return (
              ...
              <div className="form-grid">
                ...
                <div className="form-group">
                  ...
                  <input 
                    ...
                    onChange={(e) => setFormData({ ...formData, defaultSalary: e.target.value })}
                  />
                </div>

                <div className="form-group">
                  <label>Daily Fuel Cost (AED) *</label>
                  <input 
                    type="number" 
                    min="0"
                    required
                    className="form-control"
                    placeholder="e.g. 150"
                    value={formData.defaultFuel}
                    onChange={(e) => setFormData({ ...formData, defaultFuel: e.target.value })}
                  />
                </div>
              </div>
